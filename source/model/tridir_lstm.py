...
class TDLM:
    def __init__(self, config, dataset):
        ...
        inputs = [to_bein_embed, to_end_embed]
        masks = [to_begin_mask, to_end_mask]
        hidden_outputs = []
        # Create and apply LSTM
        name_surfix = ('to_begin', 'to_end')
        for i in range(len(name_surfix)):
        ...
        out_mlp = MLP(dims = [config.lstm_size*len(name_surfix)] + [config.n_labels],
                            activations = [Identity()],
                            name='out_mlp')
        out_mlp.weights_init = IsotropicGaussian(std = numpy.sqrt(2)/numpy.sqrt(config.lstm_size*len(name_surfix)+config.n_labels))
